...
def index(request):
    ...
    return  render(request, 'fraternity_tree/tree.html',
    {
...
def my_tree(request):
    brother_list = []
    tmp_list = Brother.objects.all().order_by('last_name', 'first_name', 'pledge_class__date_initiated')
...
def pledge_class(request):
    ...
    if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                pledge_brothers = Brother.objects.all().filter(pledge_class=pledge_class).order_by('last_name', 'first_name')
                return render(request,'fraternity_tree/pledge_class.html',
                {
